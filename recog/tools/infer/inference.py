...
class TextE2E(object):
    def __init__(self, args):
        ...
        self.predictor, self.input_tensor, self.output_tensors, config= utility.create_predictor(
...
if __name__ == "__main__":
    ...
    count = 0
    total_time = 0
    draw_img_save = "./recog/inference_results"
    ...
    for image_file in image_file_list:
        ...
        img_path = os.path.join(draw_img_save, 'res_imgs',
                                "e2e_res_{}".format(img_name_pure))
        text_path = os.path.join(draw_img_save, 'res_texts',
                                "e2e_res_{}".format(img_name_pure.split('.')[0]+'.txt'))
        with open(text_path, 'w', encoding='utf8') as f:
            for s in strs:f.write(s+'\n')
        cv2.imwrite(img_path, src_im)
        logger.info("The visualized image saved in {}".format(img_path))
        logger.info("The txt file saved in {}".format(text_path))
    if count > 1:
